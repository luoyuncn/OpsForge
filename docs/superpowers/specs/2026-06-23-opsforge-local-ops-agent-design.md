...
- **npm org / scope**: `@opsforge`（主包 `@opsforge/cli`；主命令 `opsforge`（无参数 → 启动 TUI），短别名 `ops`）
...
> **OpsForge 是一个 npm 可全局安装、多 provider LLM 驱动、本机优先、TUI 优先的安全运维 Agent。**
> 用户 `npm i -g @opsforge/cli` 后在终端输入 `opsforge` 即进入 **TUI**：用自然语言对话，实时看到 Forge 的**思考 → 计划 → 执行 → 验证**全过程，并在 TUI 内做**交互式审批**。它在 LLM 与"高权限执行"之间插一道**结构化安全管线**：plan → 风险门禁 → 编译 → 护栏 → 执行 → 验证 → 回滚 → 审计。CLI 子命令（`ops <cmd>`）作为脚本/CI 的次要入口保留。
...
- **TUI 优先**：`opsforge` 一进终端即开对话式 TUI，实时呈现思考/计划/执行/审批；CLI 子命令作为脚本化次要入口。
- 自然语言 → 结构化 Plan（DSL）→ 安全执行 → 验证 → 审计的完整闭环。
- 同时支持 **Linux 与 Windows 本机执行**，运行时检测当前 OS。
- **多 provider LLM**：适配 OpenAI 兼容接口（可配 base_url，覆盖 OpenAI/OpenRouter/Azure/本地 vLLM 等）、Anthropic、Google，以及 Pi 原生 provider 集；provider 可配置切换。
- 大脑用 Pi 作为 harness（session/compaction/模型路由），OpsForge 接管并守护执行，并自建 TUI 渲染层。
...
```text
┌──────────────────────────────────────────────┐
│  Entry: opsforge TUI（Ink，主入口，对话式）   │
│         + ops CLI 子命令（脚本/CI，次要）     │
│  实时渲染：思考 / 计划 / 执行 / 交互审批      │
└──────────────────────────────────────────────┘
                    │
┌──────────────────────────────────────────────┐
│  Pi Harness（@opsforge/pi-runtime, headless） │
│  session / compaction / 多 provider 模型路由  │
│  + extension: ops / policy / guard            │
│  事件流 → 上送 TUI 渲染；※ 禁用/拦截裸 bash   │
└──────────────────────────────────────────────┘
...
> 依赖前提：走 Pi/LLM 需配置任一 provider 凭证（见 §6.1：OpenAI 兼容 `OPENAI_API_KEY` + 可选 `OPENAI_BASE_URL`、`ANTHROPIC_API_KEY`、`GEMINI_API_KEY`，或 Pi 原生登录）。与 claude code / pi / codex 心智一致，可接受。Pi SDK 版本需 pin（当前 ≥0.74.x，仍在演进）。
...
Pi 以 **headless（SDK）模式**运行；TUI 由 OpsForge 自建（§7.1），通过订阅 Pi 事件流渲染。

### 6.1 多 provider 模型层

模型层 provider 无关，支持四类，配置驱动、可切换：

| provider 类型 | 配置 | 覆盖 |
|---|---|---|
| **OpenAI 兼容** | `OPENAI_API_KEY` + 可选 `OPENAI_BASE_URL` + model | OpenAI、OpenRouter、Azure OpenAI、本地 vLLM/Ollama 等任何 OpenAI 兼容端点 |
| **Anthropic** | `ANTHROPIC_API_KEY` + model | Claude 系列 |
| **Google** | `GEMINI_API_KEY` + model | Gemini 系列 |
| **Pi 原生** | Pi 登录 / AuthStorage | Pi 自带 provider 集（含订阅登录） |

- 首选复用 `@earendil-works/pi-ai`（本身即多 provider：OpenAI/Anthropic/Google 等）+ Pi 的 `ModelRegistry`/`AuthStorage`。
- 若某 provider 不被 pi-ai 覆盖，则在 `ModelProvider` 接口后加适配器（**OpenAI 兼容适配器优先**，base_url 可配即覆盖大多数第三方）。
- `opsforge config provider` 选择/切换；`opsforge doctor` 检测已配置 provider 与可用模型。
- **结构化输出兜底**：build_plan 依赖工具调用产出受 schema 约束的 Plan；对工具调用较弱的 OpenAI 兼容端点，回退 JSON-mode + zod 校验 + 重试。
...
- **禁用裸 bash**：不注册 Pi 原生自由 `bash`；若需 shell，只能经 `shell` Step（默认禁用，`--allow-shell` 开启，恒 L3 + 双重护栏 + 审计）。
- Planner = "prompt 模板 + build_plan 工具 schema + 校验"。skill 模板（§9）作为 few-shot 知识与确定性兜底喂给 planner，但主路径是 LLM。

### 6.3 事件流 → TUI

`core` 管线与 Pi session 都发出事件；`pi-runtime` 把两路事件归一成统一时间线（思考 token 流、`build_plan` 工具调用、classify/gate/compile/exec/verify/audit），供 §7.1 的 TUI 实时渲染，同时落审计。

---

## 7. 交互界面设计（TUI 优先，CLI 次要）

### 7.1 TUI 模式（主入口，`@opsforge/tui`）

终端输入 `opsforge`（无参数）→ 进入 **Ink 构建的对话式 TUI**。这是主交互阵地：

- **对话输入区**：自然语言提任务，多轮持续会话（底层 Pi session，自动 compaction）。
- **思考流**：实时流式呈现 Forge 的推理/说明（assistant token 流）。
- **计划卡片**：`build_plan` 产出的 Plan 以结构化卡片展示——标题、intent、风险徽章（L0–L3 配色）、prechecks、steps（含"将编译成的本机命令"预览）、verifications、rollback 预览。
- **交互式审批**：到风险门禁时在 TUI 内内联 approve / deny（L2 一次确认；L3 确认并填一句原因），不弹外部 prompt。
- **执行时间线**：逐步显示每个 step 的命令、stdout/stderr（截断可展开）、exit code、耗时；随后是验证结果；失败给回滚选项。
- **底部状态栏**：当前 provider/model、OS facts、提权状态、会话/审计入口。

实现：Ink + React；订阅 §6.3 的归一事件流渲染；非 TTY 环境自动降级为 CLI 模式。

### 7.2 CLI 模式（脚本/CI，次要入口）

`opsforge <subcommand>`（短别名 `ops <subcommand>`）用于脚本化、CI、非交互场景：

```text
opsforge plan "<NL>"         # 只生成并展示 Plan（dry-run，不执行）
opsforge apply <plan_id>     # 执行一个已生成的 Plan
opsforge run "<NL>"          # plan+execute 一步到位
opsforge verify <run_id>     # 重跑某次执行的验证
opsforge rollback <run_id>   # 回滚某次执行
opsforge audit ls            # 列出历史 plan/run
opsforge audit show <run_id> # 查看审计/回放（含编译命令、输出、验证、回滚）
opsforge doctor              # 自检：OS facts、包管理器、提权、provider/key、Pi 状态
opsforge config ...          # 配置 provider/base_url/key/model、风险默认、护栏开关
```

通用 flags：`--dry-run` · `--yes/-y` · `--auto-rollback` · `--risk-max <L0..L3>` · `--json` · `--provider <name>` · `--model <id>`。

非 TTY / `--json` 下的交互确认退化为：`--yes` 显式授权，否则拒绝（不阻塞挂起）。
...
opsforge/
├─ apps/
│  └─ cli/                       # @opsforge/cli → bin: opsforge(+ops 别名)；无参→挂载 TUI，有子命令→CLI
├─ packages/
│  ├─ dsl/                       # Plan/Step/Risk/Verify/Rollback（zod + 类型 + JSON Schema）
│  ├─ core/                      # 管线编排（plan→gate→compile→exec→verify→audit）
│  ├─ tui/                       # Ink TUI：对话/思考流/计划卡片/审批/执行时间线
│  ├─ planner/                   # NL→Plan：多 provider LLM adapter + skill 模板兜底
...
```text
cli → tui, core, pi-runtime, planner, config, shared
tui → core, pi-runtime, dsl, shared        # Ink/React 渲染，订阅归一事件流
...
planner → dsl, pi-ai(@earendil-works), shared   # 多 provider；OpenAI 兼容适配器可配 base_url
policy / verifier / audit → dsl, shared
```

### 第一阶段包清单（本机语义）

必做：`dsl` · `core` · `tui` · `planner` · `policy` · `verifier` · `executor-base` · `executor-linux` · `executor-windows` · `pi-runtime` · `audit` · `config` · `shared` · `cli`（共 14 个包）。
...
- **TUI：Ink + React**（主入口）；CLI 子命令解析：commander；非 TTY/简单提示兜底：`@clack/prompts`
- 执行：`child_process`（bash / PowerShell）；**不引入 `ssh2`**（本机无需）
- 存储：**better-sqlite3** + 本地文件 artifacts
- 大脑/harness：`@earendil-works/pi-coding-agent`（SDK，headless）+ `@earendil-works/pi-ai`（多 provider 模型层）
- 多 provider：OpenAI 兼容（可配 base_url）/ Anthropic / Google / Pi 原生；不被覆盖者走 `ModelProvider` 适配器
- 测试：**vitest**
- 分发：`npm i -g @opsforge/cli` → 主命令 `opsforge`（TUI）、短别名 `ops`（CLI）
...
- `planner`：经多 provider LLM（OpenAI 兼容/Anthropic/Google/Pi 原生）把 NL 变成 schema-valid Plan（含 build_plan 工具 + 校验 + 重试 + JSON-mode 兜底）。
...
- `tui`（主入口）：`opsforge` 进 TUI，实时呈现思考/计划卡片/执行时间线 + 内联交互审批。
- `cli`（次要）：`opsforge|ops plan / apply / run / verify / rollback / audit / doctor / config` + dry-run + `--json`。
- provider 配置：至少 OpenAI 兼容 + Anthropic 可跑通；`config provider` 可切换。
- 3–5 个 skill 模板（install-nginx / install-docker / install-nodejs 等）。
- `opsforge doctor` 自检通过；端到端 demo：在 TUI 里说"在本机安装 nginx" → 看到计划 → 审批 → 执行 → 验证（Linux / 等价 Windows）。
...
6. **多 provider 一致性**：不同端点的工具调用/结构化输出能力参差；OpenAI 兼容弱端点回退 JSON-mode + zod 校验 + 重试，并在 `doctor` 标注 provider 能力。
7. **TUI 复杂度**：Ink 实时渲染 + 两路事件归一是主要工程量；需保证非 TTY/CI 自动降级 CLI，且"审批暂停/恢复管线"的状态机正确。
...
- 仓库：`opsforge`
- npm scope：`@opsforge`，主包 `@opsforge/cli`
- 主命令：`opsforge`（无参数 → 启动 TUI），短别名 `ops`（CLI 子命令）
- agent 人格名（TUI 中显示）：**Forge**
- 产品名：**OpsForge**
...
| Pi | 控制面内核 | 同左，方案 C：harness（headless）+ 接管执行 |
| 存储 | Postgres/Redis/S3 | **SQLite + 本地文件** |
| 模型 | 未明确 | **多 provider**：OpenAI 兼容(可配 base_url)/Anthropic/Google/Pi 原生 |
| 形态 | 平台（cli+api+web+worker） | **单一 npm 包，TUI 优先 + CLI 子命令** |
